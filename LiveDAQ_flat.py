...
@dataclass(frozen=True)
class PulseBatch:
    pulses: tuple[LivePulse, ...]
    waveform_start_sample: int
    first_peak_sample: int
    sample_rate_hz: float
    waveform_voltage_v: tuple[float, ...]
...
class SharedMonitorState:
    def __init__(self) -> None:
        ...
        self.batches: queue.Queue[PulseBatch] = queue.Queue()
...
class ContinuousBatchBuilder:
    """Collect the uninterrupted raw Channel 1 span around a pulse batch."""

    def __init__(self, config: LiveConfig, sample_rate_hz: float) -> None:
        ...
        self.history_keep_samples = max(
            self.pretrigger_samples + self.posttrigger_samples + 16,
            round(0.025 * sample_rate_hz),
        )
        self.history: deque[DataChunk] = deque()
        self.active_start_sample: int | None = None
        self.active_next_sample: int | None = None
        self.active_voltages: list[float] = []
        self.active_pulses: list[LivePulse] = []

    def reset_after_gap(self) -> None:
        self.history.clear()
        self.active_start_sample = None
        self.active_next_sample = None
        self.active_voltages = []
        self.active_pulses = []

    def _append_chunk_to_active(self, chunk: DataChunk) -> None:
        if self.active_next_sample is None or not chunk.voltages:
            return
        chunk_end = chunk.start_sample + len(chunk.voltages)
        copy_start = max(self.active_next_sample, chunk.start_sample)
        if copy_start >= chunk_end:
            return
        offset = copy_start - chunk.start_sample
        self.active_voltages.extend(chunk.voltages[offset:])
        self.active_next_sample = chunk_end

    def ingest_chunk(self, chunk: DataChunk) -> None:
        if chunk.lost_samples:
            self.reset_after_gap()
        if not chunk.voltages:
            return
        self.history.append(chunk)
        latest_end = chunk.start_sample + len(chunk.voltages)
        keep_from = latest_end - self.history_keep_samples
        while self.history:
            first = self.history[0]
            first_end = first.start_sample + len(first.voltages)
            if first_end >= keep_from:
                break
            self.history.popleft()
        self._append_chunk_to_active(chunk)

    def _copy_history_from(self, start_sample: int) -> tuple[list[float], int]:
        copied: list[float] = []
        ...
        for chunk in self.history:
            if not chunk.voltages:
                continue
            chunk_end = chunk.start_sample + len(chunk.voltages)
            copy_start = max(start_sample, next_sample, chunk.start_sample)
            if copy_start >= chunk_end:
                continue
            if copy_start != next_sample:
                raise RuntimeError(
                    "Continuous batch history contains a sample gap; "
                    "the incomplete batch was rejected."
                )
            offset = copy_start - chunk.start_sample
            copied.extend(chunk.voltages[offset:])
            next_sample = chunk_end
        return copied, next_sample

    def add_pulse(self, pulse: LivePulse) -> PulseBatch | None:
        if self.active_start_sample is None:
            start_sample = pulse.peak_sample - self.pretrigger_samples
            try:
                voltages, next_sample = self._copy_history_from(start_sample)
            except RuntimeError:
                self.reset_after_gap()
                return None
            if not voltages:
                return None
            self.active_start_sample = start_sample
            self.active_next_sample = next_sample
            self.active_voltages = voltages
        self.active_pulses.append(pulse)
        if len(self.active_pulses) < self.num_test:
            return None

        end_sample = pulse.peak_sample + self.posttrigger_samples
        if self.active_next_sample is None or self.active_next_sample <= end_sample:
            # A LivePulse is emitted only after its post-trigger capture, so
            # this should be rare. Reject rather than display a truncated span.
            self.reset_after_gap()
            return None
        assert self.active_start_sample is not None
        sample_count = end_sample - self.active_start_sample + 1
        batch = PulseBatch(
            pulses=tuple(self.active_pulses[: self.num_test]),
            waveform_start_sample=self.active_start_sample,
            first_peak_sample=self.active_pulses[0].peak_sample,
            sample_rate_hz=self.sample_rate_hz,
            waveform_voltage_v=tuple(self.active_voltages[:sample_count]),
        )
        self.active_start_sample = None
        self.active_next_sample = None
        self.active_voltages = []
...
class AcquisitionWorker(threading.Thread):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            batch_builder = ContinuousBatchBuilder(
                self.config,
                self.source.sample_rate_hz,
            )
            output_directory = (
            ...
            while not self.stop_event.is_set():
                ...
                batch_builder.ingest_chunk(chunk)
                pulses = detector.process_chunk(chunk)
                for pulse in pulses:
                    self.logger.log(pulse)
                    self.state.events.put(pulse)
                    batch = batch_builder.add_pulse(pulse)
                    if batch is not None:
                        self.state.batches.put(batch)
                elapsed_s = (
...
class LiveDAQWindow:
    def __init__(
        ...
        self.latest_batch: PulseBatch | None = None
        self.closing = False
    ...
    def _draw_continuous_batch(self, batch: PulseBatch) -> None:
        canvas = self.canvas
        canvas.delete("all")
        ...
        voltages = batch.waveform_voltage_v
        if not voltages:
            return
        time_min = (
            batch.waveform_start_sample - batch.first_peak_sample
        ) / batch.sample_rate_hz
        time_max = (
            batch.waveform_start_sample
            + len(voltages)
            - 1
            - batch.first_peak_sample
        ) / batch.sample_rate_hz
        ...
        # Use a min/max envelope per horizontal pixel. A simple every-Nth
        # decimation can entirely skip a narrow CR-110 pulse when the five
        # event span contains hundreds of thousands of samples.
        bucket_size = max(1, math.ceil(len(voltages) / plot_width))
        points: list[float] = []
        for bucket_start in range(0, len(voltages), bucket_size):
            bucket_end = min(len(voltages), bucket_start + bucket_size)
            segment = voltages[bucket_start:bucket_end]
            minimum_offset = min(range(len(segment)), key=segment.__getitem__)
            maximum_offset = max(range(len(segment)), key=segment.__getitem__)
            for offset in sorted((minimum_offset, maximum_offset)):
                sample_offset = bucket_start + offset
                time_s = (
                    batch.waveform_start_sample
                    + sample_offset
                    - batch.first_peak_sample
                ) / batch.sample_rate_hz
                points.extend((x_of(time_s), y_of(voltages[sample_offset])))
        if len(points) >= 4:
            canvas.create_line(*points, fill="#f4d03f", width=1.5)
        ...
        for pulse_index, pulse in enumerate(batch.pulses):
            color = colors[pulse_index % len(colors)]
            peak_time_s = (
                pulse.peak_sample - batch.first_peak_sample
            ) / batch.sample_rate_hz
            peak_x = x_of(peak_time_s)
        ...
        canvas.create_text(
            ...
            height - 18,
            fill="#cbd5e1",
            text="Continuous time relative to the first pulse peak",
        )

    def refresh(self) -> None:
        if self.closing:
            return
        # Pulse summaries are still queued individually for headless mode.
        # Drain them in GUI mode so this queue cannot grow without bound.
        while True:
            try:
        ...
        while True:
            try:
                self.latest_batch = self.state.batches.get_nowait()
            except queue.Empty:
                break
            batch_updated = True
        ...
        self.result_frame.configure(
            text=(
                ...
                f"next batch {snapshot.event_count % self.num_test}/{self.num_test}"
            )
        )
        if snapshot.error:
            self.result_var.set(f"ERROR: {snapshot.error}")
        elif batch_updated and self.latest_batch is not None:
            self.result_var.set(
                "\n".join(
                    ...
                    for pulse in self.latest_batch.pulses
                )
            )
            self._draw_continuous_batch(self.latest_batch)
        elif self.latest_batch is None:
            self.result_var.set(
                f"Waiting for {self.num_test} charge pulses... "
                f"({snapshot.event_count % self.num_test}/{self.num_test})"
            )
