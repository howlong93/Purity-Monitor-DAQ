#!/usr/bin/env python3
"""Two-channel live DAQ for the purity monitor, CR-110s, and AD3.

Scope Channel 1 is the cathode/Qc channel and Scope Channel 2 is the
anode/Qa channel. Pulses are detected independently, paired by their peak-time
separation, logged, and displayed as a continuous two-channel time sequence.
...
DEFAULT_OUTPUT_ROOT = SCRIPT_DIRECTORY / "results" / "live_2c"
...
class AcquisitionWorker(threading.Thread):
    def __init__(self, source: DwfAD3Source | SimulatedSource, config: LiveConfig, state: SharedMonitorState, stop_event: threading.Event, logger: PairLogger) -> None:
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Stream AD3 Channels 1/2, pair cathode/anode pulses, and estimate Qc, Qa, and Qa/Qc.")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        window = LiveDAQ2CWindow(state, stop_event, worker, args.gui_rate, args.num_test)
        if args.duration:
